...
export function WalletConnectorCard({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className="group flex items-center justify-between rounded-none border border-border-low bg-card px-4 py-3 text-left text-sm font-medium transition hover:-translate-y-0.5 hover:shadow-sm cursor-pointer disabled:cursor-not-allowed disabled:opacity-60"
    >
      ...
      <span
        aria-hidden
        className="h-2.5 w-2.5 rounded-none bg-border-low transition group-hover:bg-primary/80"
      />
    </button>
  );
}
